...
class ClassicalPreprocessor:
    def cluster_customers_advanced(self, problem: VRPProblem, method='kmeans', max_cluster_size=3) -> Dict[int, List[int]]:
        ...
        if len(customers) == 1:
            logger.info("Only one customer to cluster, assigning to first available vehicle")
            return {problem.vehicles[0].id: customers}
        ...
        # Always use KMeans clustering
        logger.info(f"Using KMeans clustering with {n_clusters} clusters")
        clustering = KMeans(n_clusters=n_clusters, random_state=123, n_init=10)
...
class QuantumHybridSolver:
    ...
    def solve_vrp_quantum(self, problem: VRPProblem, num_runs: int = 3, clustering_method: str = 'kmeans' , choice = 'move') -> Tuple[Dict[int, List[int]], float, Dict]:
        ...
        if problem.nodes[1:]:  # Check if there are customer nodes
            try:
                current_shift = next((shift for shift, nodes in problem.shifts.items() if problem.nodes[1] in nodes), None)
                if current_shift:
                    current_shift_index = shift_order.index(current_shift)
                else:
                    logger.warning("Node 1 not found in any shift, defaulting to Shift 1")
                    current_shift_index = 0  # Default to Shift 1
            except ValueError:
                logger.warning(f"Shift not found in shift_order, defaulting to Shift 1")
        ...
        modified_routes, reassigned_customers = reassign_exceeding_customers(problem, all_routes_by_vehicle, shift_order[current_shift_index],choice)
...
def create_vrp_from_input(nodes_data: List[Dict], vehicles_data: List[Dict]):
    nodes = []
    for d in nodes_data:
        tw = tuple(d['time_window']) if 'time_window' in d and d['time_window'] else None
        node = Node(
            id=d['id'],
            x=d['x'],
            y=d['y'],
            demand=d.get('demand', 0),
            service_time=d.get('service_time', 0.5),
...
def api_solve(nodes_data: List[Dict], vehicles_data: List[Dict],choice) -> str:
    # Create full nodes and vehicles from input
    all_nodes, all_vehicles = create_vrp_from_input(nodes_data, vehicles_data)

    # For example match: TSP first 5 nodes with adjusted fields
    tsp_node_data = nodes_data[0:5]
    tsp_nodes = []
    for d in tsp_node_data:
        node = Node(
            id=d['id'],
            x=d['x'],
            y=d['y'],
    ...
    # VRP first 13 nodes, first 5 vehicles
    vrp_node_data = nodes_data[0:13]
    vrp_nodes, _ = create_vrp_from_input(vrp_node_data, vehicles_data[0:5])
    vrp_vehicles = all_vehicles[0:5]
    vrp_problem = VRPProblem(vrp_nodes, vrp_vehicles)
    vrp_problem.distance_matrix = vrp_problem._calculate_distance_matrix()
    ...
    with redirect_stdout(captured_output):
        ...
        print(f"\nTSP Problem: {tsp_problem.nodes} nodes")
        print(f"VRP Problem: {vrp_problem.nodes} nodes, {len(vrp_vehicles)} vehicles")

        print("\n" + "-"*60)
        print("TESTING: IBM Aer Simulator (MPS)")
        print("-"*60)
        vehicle_color_map = {
        ...
        print(f"\n🚚 Solving VRP ({len(vrp_nodes)} nodes, {len(vrp_vehicles)} vehicles)...")
        start_time = time.time()
        solver = QuantumHybridSolver(quantum_backend='qiskit', use_aer=True, shots=256, p_layers=1, optimizer='COBYLA')
        vrp_routes, vrp_cost, vrp_details = solver.solve_vrp_quantum(vrp_problem, num_runs=1, clustering_method='spectral',choice=choice)
        vrp_time = time.time() - start_time
        est_qubits = vrp_details.get('total_estimated_qubits', 48)
        print(f"✓ VRP Solution: Cost = {vrp_cost:.2f} km, Time = {vrp_time:.2f}s, Est. Qubits = {est_qubits}")
        ...
        print("-"*80)
        print_solution_schedules(vrp_problem, vrp_routes, "Shift 1", vrp_details.get('deferred_nodes', []))

        print_detailed_vehicle_summary(vrp_problem, vrp_routes)

        print("\n✅ Visualization saved to 'vrp_-ibm_aer_simulator(mps).html'")
        ...
        try:
            traffic_recommendations = traffic_aware_rerouting_fixed(
                vrp_problem, vrp_routes, api_key, current_positions
            )
            print(f"Vehicles analyzed: {len(traffic_recommendations)}")
            total_original_time = sum(rec.get('original_time', 0) for rec in traffic_recommendations.values())
            total_optimized_time = sum(rec.get('total_time', 0) for rec in traffic_recommendations.values())
            ...
            vehicles_with_improvements = sum(1 for rec in traffic_recommendations.values() if rec.get('status') == 'alternative')
            congestion_detected = sum(1 for rec in traffic_recommendations.values() if rec.get('congested_segments'))
            print(f"Vehicles with congestion detected: {congestion_detected}")
            ...
            if vehicles_with_improvements == 0:
                print("\n💡 Analysis: No route improvements found because:")
                print("   • Alternative routes had similar or worse travel times")
                print("   • Congestion detected but alternatives not significantly better")
                print("   • Traffic conditions may not warrant major route changes")
        except Exception as traffic_error:
            print(f"⚠ Traffic analysis failed: {traffic_error}")
        print("EXPERIMENT RESULTS SUMMARY")
        print("="*80)
        print(f"{'Configuration':<25} VRP Cost   | VRP Time   | VRP Est. Qubits")
        print("-" * 70)
        print(f"{'IBM Aer Simulator (MPS)':<25} {vrp_cost:<10} | {vrp_time:<10.2f}s     | {est_qubits}")
...
def solve_vrp_from_api(nodes_data, vehicles_data):
    ...
    # Convert input data to Node and Vehicle objects
    nodes = [Node(
        id=n['id'],
        x=n['x'],
        y=n['y'],
    ...
    # Create VRP problem from user input
    vrp_problem = VRPProblem(nodes=nodes, vehicles=vehicles, shifts=group_nodes_by_shift(nodes))
    
    print("🔍 Checking IBM Quantum connectivity...")
    check_quantum_connectivity()  # This will print the backend details
    ...
    # Print detailed problem info
    print(f"VRP Problem: {nodes} nodes, {len(vehicles)} vehicles\n")
    vehicle_color_map = {
        0: {'color': 'red', 'label': 'Truck (60kg)'},
        1: {'color': 'blue', 'label': 'Car 1 (30kg)'},
        2: {'color': 'green', 'label': 'Car 2 (30kg)'},
    ...
    for config in test_configs:
        ...
        try:
            ...
            print(f"🚚 Solving VRP ({vrp_problem.n_nodes} nodes, {vrp_problem.n_vehicles} vehicles)...")
        ...
        for shift_name in ["Shift 1", "Shift 2", "Shift 3"]:
            ...
            try:
                ...
                print(f"✓ VRP Solution: Cost = {vrp_cost:.2f} km, Time = {vrp_time:.2f}s, Est. Qubits = {config_results['vrp'][shift_name]['total_estimated_qubits']}")
                print(f"  Routes: {vrp_routes}")
                print(f"  Clusters: {vrp_details.get('clusters', [])}")
                ...
                try:
                    ...
                    logger.error(f"Traffic analysis failed for {shift_name}: {traffic_error}")
                    print(f"⚠ Traffic analysis failed for {shift_name}: {traffic_error}")
                    traffic_recommendations = None

                if traffic_recommendations:
                    print(f"\n{'=' * 80}")
                    print(f"ENHANCED TRAFFIC OPTIMIZATION SUMMARY FOR {shift_name}")
                    print(f"{'=' * 80}")

                    total_original_time = sum(rec.get('original_time', 0)
                                              for rec in traffic_recommendations.values())
                    total_optimized_time = sum(rec.get('total_time', 0)
                                               for rec in traffic_recommendations.values())

                    vehicles_with_improvements = sum(1 for rec in traffic_recommendations.values()
                                                     if rec.get('status') == 'alternative')

                    total_time_saved = total_original_time - total_optimized_time
                    efficiency_gain = ((total_time_saved / total_original_time * 100)
                                       if total_original_time > 0 else 0)

                    congestion_detected = sum(1 for rec in traffic_recommendations.values()
                                              if rec.get('congested_segments'))
                ...
                if vrp_routes:
                    ...
                    visualize_solution_with_folium_fixed(shift_vrp_problem, vrp_routes, f"VRP - {config['name']} - {shift_name}", vehicle_color_map)
                    try:
                        ...
                        logger.error(f"Visualization failed for {shift_name}: {viz_error}")
                        print(f"⚠ Visualization generation failed for {shift_name}: {viz_error}")
            except Exception as e:
    ...
    print("EXPERIMENT RESULTS SUMMARY")
    print(f"{'=' * 80}\n")
    print(f"{'Configuration':<25} | {'TSP Cost':<10} | {'TSP Time':<10} | {'TSP Qubits':<12} | {'VRP Cost':<10} | {'VRP Time':<10} | {'VRP Est. Qubits':<15}")
    print("-" * 120)
    ...
    for config_name, config_results in results.items():
        ...
        # Get the results for the first shift or main VRP result
        vrp_result = None
        if config_results.get('vrp'):
            if isinstance(config_results['vrp'], dict) and 'success' in config_results['vrp']:
                # Direct VRP result
                vrp_result = config_results['vrp']
            elif "Shift 1" in config_results['vrp']:
                # First shift result
                vrp_result = config_results['vrp']["Shift 1"]
        
        if vrp_result and vrp_result.get('success'):
            vrp_cost = f"{vrp_result['cost']:.2f}"
            vrp_time = f"{vrp_result['time']:.2f}s"
            vrp_qubits = vrp_result['total_estimated_qubits']
        
        print(f"{config_name:<25} | {tsp_cost:<10} | {tsp_time:<10} | {str(tsp_qubits):<12} | {vrp_cost:<10} | {vrp_time:<10} | {str(vrp_qubits):<15}")
...
def reassign_exceeding_customers(
    problem: VRPProblem, routes: Dict[int, List[int]], current_shift: str, choice: str
) -> Tuple[Dict[int, List[int]], List[int]]:
    """
    Checks routes for time limit violations. If a violation occurs, it identifies the last
    customer in the route and applies the user's choice to reassign them to the next shift or allow overtime.
    ...
    Args:
        ...
        choice: User choice ("overtime" or "move") for handling exceeding customers.
...
def suggest_alternative_route_fixed(problem: 'VRPProblem', vehicle: Vehicle, route: List[int],
    ...
    for cong_seg in congested_segments:
        ...
        if alt_duration and alt_duration < cong_seg['traffic_adjusted_duration'] * 0.9:  # 10% improvement threshold
...
def traffic_aware_rerouting_fixed(problem: 'VRPProblem', routes_by_vehicle: Dict[int, List[int]],
    ...
    """
    Enhanced to print street names and directions for alternative routes.
    """
    ...
    for vehicle_id, route in routes_by_vehicle.items():
        ...
        recommendation = suggest_alternative_route_enhanced(problem, vehicle, route, api_key, current_node_idx)
        ...
        if recommendation['status'] == 'alternative':
            ...
            if 'alternatives_found' in recommendation:
                print(f"Segments with alternative routes: {len(recommendation['alternatives_found'])}")
                for alt in recommendation['alternatives_found']:
                    if 'segment' in alt:
                        seg = alt['segment']
                        print(f"  • Alternative for {seg['start_node']}→{seg['end_node']}: {alt['time_saved']:.2f}h saved")
                    elif 'segment_group' in alt:
                        print(f"  • Group alternative for {alt['group_start']}→{alt['group_end']}: {alt['time_saved']:.2f}h saved")
                    print("    Route Instructions:")
                    for step in alt.get('steps', []):
                        print(f"      - {step['maneuver']} (Distance: {step['distance']:.2f}km)")
        elif recommendation['status'] == 'original_with_congestion':
            ...
            for seg in recommendation['congested_segments']:
                ...
                print(f"    Reasons: {', '.join(seg.get('congestion_reasons', ['unknown']))}")
        else:
            print("No congestion or alternatives found.")
...
def download_vrp_visualizations(problem, routes_by_vehicle, shift_name, vehicle_color_map=None, traffic_recommendations=None):
    ...
    if traffic_recommendations:
        ...
        if optimized_routes:
            ...
            visualize_solution_with_folium_fixed(
                problem,
                optimized_routes,
                optimized_title,
                vehicle_color_map,
        ...
        try:
            ...
            if use_alternative and len(routes) > 1:
                ...
                if best_alt and max_difference > 0.1:  # At least 10% different
                    ...
                    steps = best_alt.get('legs', [{}])[0].get('steps', [])
                else:
            ...
            # Extract street names and maneuvers from steps
            step_details = []
            for step in steps:
                street_name = step.get('name', 'Unnamed road')
                maneuver = step.get('maneuver', {}).get('type', 'N/A')
                direction = step.get('maneuver', {}).get('modifier', '')
                step_details.append({
                    'street_name': street_name,
                    'maneuver': f"{maneuver} {'onto ' + street_name if street_name != 'Unnamed road' else ''} {direction}".strip(),
                    'distance': step.get('distance', 0) / 1000.0,  # Convert to km
                    'duration': step.get('duration', 0) / 3600.0   # Convert to hours
                })

            return [(lat, lon) for lon, lat in route_geometry], step_details
...
def visualize_solution_with_folium_fixed(problem, routes_by_vehicle, title,
    ...
    """
    Enhanced to show street names and directions in popups for alternative routes.
    """
    depot_node = problem.nodes[0]
    map_center = [depot_node.y, depot_node.x]
    ...
    for vehicle_id, route in routes_by_vehicle.items():
        ...
        if traffic_recommendations and vehicle_id in traffic_recommendations:
            ...
            has_alternatives = (rec.get('status') == 'alternative' and
                              'alternatives_found' in rec and rec['alternatives_found'])
            if has_alternatives:
                for alt in rec['alternatives_found']:
                    if 'segment' in alt:
                        seg = alt['segment']
                        key = (seg['start_node'], seg['end_node'])
                        alternative_steps[key] = alt.get('steps', [])
                    elif 'segment_group' in alt:
                        key = (alt['group_start'], alt['group_end'])
                        alternative_steps[key] = alt.get('steps', [])
        ...
        for j in range(len(viz_route) - 1):
            ...
            if has_alternatives:
                rec = traffic_recommendations[vehicle_id]
                for alt in rec.get('alternatives_found', []):
                    if 'segment' in alt and alt['segment']['start_node'] == start_node_id and \
                       alt['segment']['end_node'] == end_node_id:
                        use_alternative_path = True
                        segment_label_suffix = " (Optimized Route)"
                        segment_steps = alt.get('steps', [])
                        break
                    elif 'segment_group' in alt and alt['group_start'] == start_node_id and \
                         alt['group_end'] == end_node_id:
                        use_alternative_path = True
                        segment_label_suffix = " (Optimized Group Route)"
                        segment_steps = alt.get('steps', [])
                        break
            ...
            # Get route geometry and steps
            path_geometry, steps = get_route_geometry_from_osrm_fixed(
                start_node, end_node, use_alternative=use_alternative_path
            )

            # Prepare popup content with street names and directions
            popup_content = f"<strong>{route_label}: {start_node_id}→{end_node_id}{segment_label_suffix}</strong><br>"
            popup_content += f"Distance: {problem.distance_matrix[start_node_id][end_node_id]:.2f}km<br>"
            if use_alternative_path and segment_steps:
                popup_content += "<br><strong>Route Instructions:</strong><ul>"
                for step in segment_steps:
                    popup_content += f"<li>{step['maneuver']} ({step['distance']:.2f}km)</li>"
                popup_content += "</ul>"
            ...
            if path_geometry:
                ...
                # Fallback: draw straight line if OSRM fails
                logger.warning(f"Drawing straight line for {start_node_id}->{end_node_id}")
                popup_content = f"<strong>{route_label}: {start_node_id}→{end_node_id} (Direct)</strong><br>"
                popup_content += f"Distance: {problem.distance_matrix[start_node_id][end_node_id]:.2f}km<br>"
                popup_content += "No detailed route available"
                folium.PolyLine(
                    locations=[[start_node.y, start_node.x], [end_node.y, end_node.x]],
                    color=route_color,
                    weight=2,
    ...
    if traffic_recommendations:
        legend_html += "<p><b>Legend Notes:</b></p>"
        legend_html += "<p>Thicker lines indicate optimized (alternative) routes</p>"
        legend_html += "<p>Click routes for street names and directions</p>"
...
def suggest_alternative_route_enhanced(problem, vehicle, route, api_key, current_node_idx):
    """
    Enhanced to include street names and maneuver directions for alternative routes.
    """
    if current_node_idx < 0 or current_node_idx >= len(route):
        return {'route': route, 'total_time': float('inf'), 'status': 'invalid_index'}
    ...
    for group in congested_groups:
        if len(group) == 1:
            ...
            # Get alternative route with steps
            slow_points = [(start_node.y, start_node.x)]
            alt_geometry, alt_steps = get_route_geometry_from_osrm_fixed(start_node, end_node, use_alternative=True)
            alt_distance = sum(step['distance'] for step in alt_steps) if alt_steps else None
            alt_duration = sum(step['duration'] for step in alt_steps) if alt_steps else None
            ...
            if alt_duration and alt_duration < seg['traffic_adjusted_duration'] * 0.95:
                ...
                alternatives_found.append({
                    ...
                    'steps': alt_steps  # Include step-by-step instructions
                })
                total_time_saved += time_saved
                logger.info(f"Alternative found: {seg['start_node']}→{seg['end_node']} "
                           f"saves {time_saved:.2f}h")
                # Log street names and directions
                for step in alt_steps:
                    logger.info(f"  Step: {step['maneuver']} (Distance: {step['distance']:.2f}km)")
        else:
